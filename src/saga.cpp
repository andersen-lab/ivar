...
void merge_reads(const bam1_t* read1, const bam1_t* read2, IntervalTree &amplicons, uint8_t min_qual, ref_antd &refantd, std::string ref_name, std::vector<genomic_position> &global_positions){
  ...
  if(start_forward < end_reverse && end_forward < end_reverse){
    amplicons.find_read_amplicon(start_forward, end_reverse, nodes, amp_dist);
  }
  add_variants(final_positions, final_bases, final_qualities, global_positions);
  if(nodes.size() == 1){
    assign_read(nodes[0], final_positions, final_bases, final_qualities, global_positions);
  }else if(nodes.size() > 1){
    for(auto node : nodes){
      mark_ambiguous_reads(node, final_positions, global_positions);
    }
  }
}
...
int preprocess_reads(std::string bam, std::string bed, std::string bam_out, std::string cmd, std::string pair_info, int32_t primer_offset, uint32_t min_depth, uint8_t min_qual, std::string ref_file){
  ...
  while (sam_read1(in, header, aln) >= 0) {
    ...
    if (!(aln->core.flag & BAM_FPAIRED) || !(aln->core.flag & BAM_FPROPER_PAIR)){
      ...
      uint32_t amp_dist = 0;
      std::vector<ITNode*> nodes;
      amplicons.find_read_amplicon(start_read, end_read, nodes, amp_dist);
      add_variants(positions, bases, qualities, global_positions);
      if(nodes.size() == 1){
        assign_read(nodes[0], positions, bases, qualities, global_positions);
      }else if (nodes.size() > 1){
        for(auto node : nodes){
          mark_ambiguous_reads(node, positions, global_positions);
        }
      }
      continue;
    }
