...
int main(int argc, char *argv[]) {
  ...
  if (cmd.compare("contam") == 0) {
    ...
    if (!g_args.variants.empty() && !g_args.prefix.empty()) {
      ...
      std::vector<variant> variants = gmm_model(g_args.variants, g_args.prefix, g_args.min_depth, g_args.min_qual, solution, means, std_devs, g_args.ref, g_args.min_threshold);
      cluster_consensus(variants, g_args.prefix, g_args.min_threshold, g_args.min_depth, g_args.min_qual, solution, means, std_devs, g_args.ref);
    }
    res = 0;
